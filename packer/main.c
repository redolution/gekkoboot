...
extern const uint8_t executable_xz[];
// Symbols fed in from the linker script,
// we really want the "address" and not the non-existent value they would point to
extern const uint8_t executable_xz_size[];
extern const uint8_t executable_bin_size[];

static struct xz_buf xz_buf = {
	.in = executable_xz,
	.in_size = (size_t) executable_xz_size,
	.out = EXECUTABLE_ADDR,
	.out_size = (size_t) executable_bin_size
};
